<?php
	...
	$purchases = $db->query("SELECT * FROM purchase");
	$currentPurchase = $purchases->fetch_row();
	$response = array();
	
	while($currentPurchase) {
		$purchaseId = $currentPurchase[0];
		$username = $currentPurchase[1];
		$departmentName = $db->query("SELECT department_name FROM doctor WHERE username = '" . $username . "'")->fetch_object()->department_name;
		...
		while($currentInhalableOrder) {
			...
			$order = array('purchaseId' => $purchaseId, 'departmentName' => $departmentName, 'dateTime' => $dateTime, 'drugId' => $drugId, 'drugName' => $drugName, 'isInhalable' => $isInhalable, 'category' => $category, 'flowLitersPerMinute' => $flowLitersPerMinute, 'inConcentrationPercent' => $inConcentrationPercent, 'numMinutes' => $numMinutes, 'cost' => $cost);
		...
		while($currentNonInhalableOrder) {
			...
			$order = array('purchaseId' => $purchaseId, 'departmentName' => $departmentName, 'dateTime' => $dateTime, 'drugId' => $drugId, 'drugName' => $drugName, 'isInhalable' => $isInhalable, 'category' => $category, 'numUnits' => $numUnits, 'cost' => $cost);
			array_push($response, $order);
			$currentNonInhalableOrder = $nonInhalableOrders->fetch_row();
		}
